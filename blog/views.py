...
def crear_publicacion(request):
    ...
    if request.method == "POST":
        publicacion = CrearPublicacion(request.POST, request.FILES) #Aqui llega la informacion
        if publicacion.is_valid(): # Si pasa la validacion de Django hace lo si
        ...
        publicacion = CrearPublicacion() #Formulario vacion para construir el HTML
        imagen=Avatar
...
def leerpublicacion(request, id):
    ...
    return render (request, 'blog/leerpublicacion.html', {'leerpublicacion': leerpublicacion,'imagen':obtenerAvatar(request)})
